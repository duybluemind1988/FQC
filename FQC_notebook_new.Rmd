...
#file='/media/ad/01D6B57CFBE4DB20/1.Linux/Data/FQC/V04R-V04R-SQLData_4000.dat' 
...
library(pastecs)
stat.desc(df_plot %>% select(parameter_freq,parameter_value))
```
```{r}
#install.packages('Hmisc')
library(Hmisc)
describe(df_plot %>% select(parameter_freq,parameter_value))
```
...
#**Remove outlier**
```{r}
select_data<-df_plot$parameter_value
Q1 <- quantile(select_data, .25)
Q3 <- quantile(select_data, .75)
IQR <- IQR(select_data)
k=3
df_plot_no_outlier <- subset(df_plot, select_data> (Q1 - k*IQR) & select_data< (Q3 + k*IQR))
...
```{r}
# Plot value only:
plot_dygraph <- dygraph(df_plot %>% select(date_trans,parameter_value)) %>%
...
```{r}
# Plot value only:
plot_dygraph <- dygraph(df_plot_no_outlier %>% select(date_trans,parameter_value)) %>%
  dyOptions( drawPoints = TRUE, pointSize = 4,useDataTimezone = TRUE)%>% 
  dyRangeSelector()
plot_dygraph
```
...
dy_graph <- list(
  dygraph(df_plot_no_outlier %>% select(date_trans,parameter_value), main = "db", group = "chart")%>%
  dyOptions( drawPoints = TRUE, pointSize = 4,useDataTimezone = TRUE ),
dygraph(df_plot_no_outlier %>% select(date_trans,parameter_freq), main = "Frequency", group = "chart")%>%
  dyOptions( drawPoints = TRUE, pointSize = 4,useDataTimezone = TRUE )
)  # end list
...
# Hide blank space with no x-values 
```{r}
data_research
```
